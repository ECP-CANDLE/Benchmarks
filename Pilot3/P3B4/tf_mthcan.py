...
class hcan(object):
    def __init__(
        ...
        # doc input
        self.doc_input = tf.compat.v1.placeholder(
            tf.int32, shape=[None, max_sents, max_words]
        )  # batch x sents x words
        ...
        doc_input_reshape = tf.reshape(
            doc_input_reduced, (-1, max_words_)
        )  # batch*sents x words
        ...
        word_embeds = tf.nn.dropout(
            word_embeds, self.dropout
        )  # batch*sents x words x attention_size

        # word self attention
        Q = tf.compat.v1.layers.conv1d(
            word_embeds,
            self.attention_size,
            1,
            padding="same",
        ...
        sent_embeds = tf.nn.dropout(
            sent_embeds, self.dropout
        )  # batch x sents x attention_size
        ...
        doc_embeds = tf.nn.dropout(
            tf.squeeze(outputs, [1]), self.dropout
        )  # batch x attention_size

        # classification functions
        logits = []
        self.predictions = []
        for i in range(self.num_tasks):
            logit = tf.compat.v1.layers.dense(
                doc_embeds,
                num_classes[i],
                kernel_initializer=tf.initializers.glorot_uniform(),
            )
        ...
        if int(tf_version_split[0]) == 1 and int(tf_version_split[1]) > 13:
            self.optimizer = tf.train.experimental.enable_mixed_precision_graph_rewrite(
    ...
    def train(self, data, labels, batch_size=100, epochs=50, validation_data=None):
        ...
        for ep in range(epochs):
            ...
            for start in range(0, len(data), batch_size):
                ...
                feed_dict = {
                    self.doc_input: data[start:stop],
                    self.dropout: self.dropout_keep,
                }
                for i in range(self.num_tasks):
                    feed_dict[self.labels[i]] = labels[i][start:stop]
                retvals = self.sess.run(
                    self.predictions + [self.optimizer, self.loss], feed_dict=feed_dict
                )
                loss = retvals[-1]
            ...
            for i in range(self.num_tasks):
                ...
                print(
                    ...
                    % (ep + 1, i + 1, micro, macro)
                )

            scores, val_loss = self.score(
