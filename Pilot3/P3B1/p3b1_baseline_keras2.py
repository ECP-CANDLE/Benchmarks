...
def initialize_parameters():
    ...
    p3b1Bmk = benchmark.BenchmarkP3B1(benchmark.file_path, 'p3b1_default_model.txt', 'keras',
    prog='p3b1_baseline', desc='Multi-task (DNN) for data extraction from clinical reports - Pilot 3 Benchmark 1')
    
    # Initialize parameters
    gParameters = default_utils.initialize_parameters(p3b1Bmk)
...
def load_data(gParameters):
    ...
    fpath = default_utils.fetch_file(path + gParameters['train_data'], 'Pilot3', untar=True)
...
def run(gParameters, fpath):
    ...
    for fold in range( gParameters['n_fold'] ):
        ...
        # build data
        X_train, Y_train, X_test, Y_test = benchmark.build_data( len(individual_nnet_spec), fold, fpath )
