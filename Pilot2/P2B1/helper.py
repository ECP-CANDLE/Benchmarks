...
def get_local_files(
    data_tag="3k_run16", data_dir_prefix="/p/gscratchr/brainusr/datasets/cancer/pilot2"
):
    """
...
def append_nbrs_invariant(x, nbrs, num_nbrs):
    ...
    for i in range(len(x)):
        ...
        orientation = np.squeeze(
            orientationVector(x[i, 3, :2].reshape(1, 1, -1), x[i, 2, :2], [1.0, 1.0])
        )

        # Calculate relative periodic distances from the com
        temp_mols[:, :, 0] = periodicDistance(xy_feats, com[0, :2], [1.0, 1.0])

        temp_mols[:, :, 1] = get_angles(xy_feats, com[0, :2], orientation, [1.0, 1.0])
...
def append_nbrs_relative(x, nbrs, num_nbrs):
    ...
    for i in range(len(x)):
        ...
        # Calculate relative periodic distances from the com
        temp_mols = periodicVector(
            temp_mols, com, [1.0, 1.0, 0.3]
        )  # The absolute span of z-dimension is about third of x and y
