...
def full_conv_mol_auto(
    ...
    for i, l in enumerate(hidden_layers):
        if i == 0:
            encoded = Convolution2D(
                l,
                bead_k_size,
                strides=(1, bead_k_size),
                padding="same",
            ...
            encoded = Convolution2D(
                l,
                input_shape[1] / (mol_k_size * bead_k_size),
                strides=(1, input_shape[1] / (mol_k_size * bead_k_size)),
                padding="same",
                activation=nonlinearity,
                kernel_regularizer=l2(l2_reg),
                kernel_initializer="glorot_normal",
...
def dense_auto(
    ...
    for i, l in reversed(list(enumerate(hidden_layers))):
        if i < len(hidden_layers) - 1:
            if i == len(hidden_layers) - 2:
                decoded = Dense(
                    l, activation=nonlinearity, kernel_regularizer=l2(l2_reg)
                )(encoded)
                decoded = BatchNormalization()(decoded)
                decoded = Dropout(drop)(decoded)
            else:
