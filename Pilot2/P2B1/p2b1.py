...
class autoencoder_preprocess:
    def __init__(self, img_size=(784,), noise_factor=0.0):
...
class Candle_Molecular_Train:
    ...
    def datagen(self, epoch=0, print_out=1, test=0):
        ...
        if not test:
            order = random.sample(
                list(self.train_ind), int(self.sampling_density * len(self.train_ind))
            )
        else:
            order = self.test_ind
        ...
        for i in range(1, self.mb_epochs + 1):
            ...
            for curr_file, xt_all, yt_all in self.datagen(i):
                ...
                for frame in range(len(xt_all)):
                    history = self.molecular_model.fit(
                        xt_all[frame],
                        yt_all[frame],
                        epochs=1,
                        batch_size=self.batch_size,
                        callbacks=self.callbacks[:2],
                    )
            ...
            for curr_file, xt_all, yt_all in self.datagen(0, 0, test=1):
                ...
                for frame in range(len(xt_all)):
                    ...
                    yp = self.molecular_encoder.predict(
                        xt_all[frame], batch_size=self.batch_size
                    )
                    XP.append(yp)
                ...
                fout = (
                    current_path
                    + "/"
                    + curr_file.split("/")[-1].split(".npz")[0]
                    + "_AE"
                    + "_Include%s" % self.type_feature
                    + "_Conv%s" % self.conv_net
                    + ".npy"
                )
                print(fout)
                np.save(fout, XP)
