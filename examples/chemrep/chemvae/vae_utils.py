...
class VAEUtils(object):
    def __init__(
        self, exp_file="exp.json", encoder_file=None, decoder_file=None, directory=None
    ):
        # files
    ...
    def prep_mol_df(self, smiles, z):
        ...
        sort_df = pd.DataFrame(
            df[["smiles"]].groupby(by="smiles").size().rename("count").reset_index()
        )
    ...
    def smiles_to_hot(self, smiles, canonize_smiles=True, check_smiles=False):
        ...
        p = self.params
        z = mu.smiles_to_hot(
            smiles, p["MAX_LEN"], p["PADDING"], self.char_indices, p["NCHARS"]
        )
        return z
