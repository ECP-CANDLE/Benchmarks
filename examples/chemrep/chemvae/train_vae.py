...
def vectorize_data(params):
    ...
    if np.shape(X)[0] % params["batch_size"] != 0:
        ...
        if params["do_prop_pred"]:
            if "reg_prop_tasks" in params:
                Y_reg = Y_reg[
                    : np.shape(Y_reg)[0] // params["batch_size"] * params["batch_size"]
                ]
            if "logit_prop_tasks" in params:
                Y_logit = Y_logit[
                    : np.shape(Y_logit)[0]
    ...
    print(
        "Training set size is {}, after filtering to max length of {}".format(
...
def load_models(params):
    ...
    z_samp, z_mean_log_var_output = variational_layers(
        z_mean, enc_output, kl_loss_var, params
    )
...
def kl_loss(truth_dummy, x_mean_log_var_output):
    ...
    kl_loss = -0.5 * K.mean(
        1 + x_log_var - K.square(x_mean) - K.exp(x_log_var), axis=-1
    )
