        ...
        for i in range(2, params["middle_layer"] + 1):
            ...
            if params["batchnorm_mid"]:
                middle = BatchNormalization(
                    axis=-1, name="encoder_dense{}_norm".format(i)
                )(middle)
    else:
        middle = x
...
def variational_layers(z_mean, enc, kl_loss_var, params):
    ...
    def sampling(args):
        ...
        epsilon = K.random_normal_variable(
            shape=(params["batch_size"], params["hidden_dim"]), mean=0.0, scale=1.0
        )
        # insert kl loss here
...
def property_predictor_model(params):
    ...
    prop_mid = Dense(
        params["prop_hidden_dim"], activation=params["prop_pred_activation"]
    )(ls_in)
