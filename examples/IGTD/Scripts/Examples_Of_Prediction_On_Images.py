...
import numpy as np
from Prediction_Modeling_Functions import (
...
sampleID["valID"] = rand_sample_ID[
    range(fold_size * (num_fold - 2), fold_size * (num_fold - 1))
]
...
predResult, perM, perf, winningModel, batch_size = CNN2D_Regression_Analysis(
    train=train, resultFolder=result_dir, para=para, val=val, test=test
)

result = {}
result[
    "predResult"
] = predResult  # Prediction values for the training, validation, and testing sets
...
perf.to_csv(
    result_dir + "/Prediction_Performance.txt",
    header=True,
    index=True,
    sep="\t",
    line_terminator="\r\n",
...
predResult, perM, perf, winningModel, batch_size = CNN2D_Classification_Analysis(
    train=train,
    num_class=2,
