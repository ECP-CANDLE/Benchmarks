...
def load_data(params, seed):
    ...
    if params['in'].endswith('h5') or params['in'].endswith('hdf5'):
        ...
        df_x_train_0 = pd.read_hdf(params['in'], 'x_train_0').astype(np.float32)
        df_x_train_1 = pd.read_hdf(params['in'], 'x_train_1').astype(np.float32)
        X_train = pd.concat([df_x_train_0, df_x_train_1], axis=1, sort=False)
        del df_x_train_0, df_x_train_1

        df_x_test_0 = pd.read_hdf(params['in'], 'x_test_0').astype(np.float32)
        df_x_test_1 = pd.read_hdf(params['in'], 'x_test_1').astype(np.float32)
        X_test = pd.concat([df_x_test_0, df_x_test_1], axis=1, sort=False)
        del df_x_test_0, df_x_test_1

        df_x_val_0 = pd.read_hdf(params['in'], 'x_val_0').astype(np.float32)
        df_x_val_1 = pd.read_hdf(params['in'], 'x_val_1').astype(np.float32)
        X_val = pd.concat([df_x_val_0, df_x_val_1], axis=1, sort=False)
        del df_x_val_0, df_x_val_1

        Y_train = pd.read_hdf(params['in'], 'y_train')
        Y_test = pd.read_hdf(params['in'], 'y_test')
        Y_val = pd.read_hdf(params['in'], 'y_val')
