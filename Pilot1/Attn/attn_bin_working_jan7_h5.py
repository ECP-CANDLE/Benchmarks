...
checkpointer = ModelCheckpoint(
    filepath=args["save_dir"] + "Agg_attn_bin.autosave.model.h5",
    verbose=1,
...
plt.plot(
    recall_keras, precision_keras, label="PR Keras (area = {:.3f})".format(pr_keras)
)
plt.xlabel("Recall")
...
print(
    "creating figure 4 at ",
    args["save_dir"] + "Agg_attn_bin.confusion_without_norm.pdf",
)
plot_confusion_matrix(
    cnf_matrix, classes=class_names, title="Confusion matrix, without normalization"
)
plt.savefig(
    args["save_dir"] + "Agg_attn_bin.confusion_without_norm.pdf", bbox_inches="tight"
)
...
plt.savefig(
    args["save_dir"] + "Agg_attn_bin.confusion_with_norm.pdf", bbox_inches="tight"
)
...
np.savetxt(
    args["save_dir"] + "Agg_attn_bin_predict_train.csv",
    predict_train,
    delimiter=",",
    fmt="%.3f",
)
np.savetxt(
    args["save_dir"] + "Agg_attn_bin_predict_test.csv",
    predict_test,
    delimiter=",",
    fmt="%.3f",
)
...
np.savetxt(
    args["save_dir"] + "Agg_attn_bin_predict_train_classes.csv",
    predict_train_classes,
    delimiter=",",
    fmt="%d",
)
np.savetxt(
    args["save_dir"] + "Agg_attn_bin_predict_test_classes.csv",
