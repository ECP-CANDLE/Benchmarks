...
def initialize_hvd():
    ...
    #HVD-2: GPU pinning
    gpus = tf.config.experimental.list_physical_devices('GPU')#XPU for PVC
...
def inference_data_gen(hyper_params, tokenizer, fil, rank):
    ...
    #print(x_inference)
    del(Data_smiles_raw)
    #x_inference = tf.data.Dataset.from_tensor_slices(Data_smiles).batch(BATCH).prefetch(buffer_size = 1).map(lambda x: tf.cast(tokenizer(x), dtype=tf.int16), num_parallel_calls=4)
    # x_inference = x_inference.batch(BATCH).prefetch(buffer_size = tf.data.AUTOTUNE)
    #x_inference.cache('cache_rank{rank}')
