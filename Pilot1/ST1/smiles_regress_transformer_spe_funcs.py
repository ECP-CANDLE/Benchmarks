...
def preprocess_smiles_pair_encoding(data, maxlen, vocab_file, spe_file):
    ...
    tokenized_data = np.array(
        [list(pad(tokenizer(smi)["input_ids"], maxlen, 0)) for smi in data]
    )
...
def train_val_data(hyper_params):
    ...
    if tokenizer_params["category"] == "smilespair":
        ...
        x_train = preprocess_smiles_pair_encoding(
            x_smiles_train, maxlen, vocab_file, spe_file
        )

        x_val = preprocess_smiles_pair_encoding(
            x_smiles_val, maxlen, vocab_file, spe_file
        )
        print(x_train)
...
class ModelArchitecture(layers.Layer):
    ...
    def __init__(self, hyper_params):
        ...
        self.transformer_block = TransformerBlock(
            embed_dim, num_heads, ff_dim, DR_TB_1, activation_transformer, dropout1
        )
...
class TrainingAndCallbacks:
    ...
    def __init__(self, hyper_params):
        ...
        self.clr = CyclicLR(base_lr=lr, max_lr=5 * lr, step_size=2000.0)
...
class RunFnDeepHyper:
    ...
    def run(self, config):
        ...
        history = TrainingAndCallbacks(
            self.hvd_switch,
            self.checkpt_file,
            lr,
            self.csv_file,
            self.patience_red_lr,
            self.patience_early_stop,
        ).training(
            model, self.x_train, self.y_train, validation_data, self.BATCH, EPOCH
        )
...
def run(config):
    ...
    x_train, y_train, x_val, y_val = train_val_data(
        data_path_train, data_path_vali, hvd_switch, vocab_size, maxlen
    )
    num_heads = config["num_heads"]
    DR_TB = config["DR_TB"]
    DR_ff = config["DR_ff"]
    ...
    ).training(model, x_train, y_train, validation_data, BATCH, EPOCH)
