...
def get_drug_fgpt_df(data_root: str, int_dtype: type = np.int8):
    ...
    # Otherwise load from raw files, process it and save ######################
        ...
        download_files(
            filenames=[ECFP_FILENAME, PFP_FILENAME],
...
def get_drug_dscptr_df(
    ...
    # Otherwise load from raw files, process it and save ######################
        ...
        # Download the raw file if not exist
        download_files(
            filenames=DSCPTR_FILENAME, target_folder=os.path.join(data_root, RAW_FOLDER)
        )
...
def get_drug_feature_df(
    ...
    if drug_feature_usage == "both":
        return pd.concat(
            [
                get_drug_fgpt_df(data_root=data_root, int_dtype=int_dtype),
                get_drug_dscptr_df(
                    data_root=data_root,
...
def get_drug_target_df(data_root: str, int_dtype: type = np.int8):
    ...
    df["TARGET"] = encode_label_to_int(
        data_root=data_root, dict_name="drug_target_dict.txt", labels=df["TARGET"]
    )
...
def get_drug_qed_df(data_root: str, qed_scaling: str, float_dtype: type = np.float32):
