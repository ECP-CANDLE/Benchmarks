...
def get_drug_resp_df(
    ...
    # Otherwise load from raw files, process it and save ######################
        ...
        df[["LOG_CONCENTRATION", "GROWTH"]] = df[
    ...
    df[["LOG_CONCENTRATION", "GROWTH"]] = df[["LOG_CONCENTRATION", "GROWTH"]].astype(
        float_dtype
    )
    return df
...
def get_combo_stats_df(
    ...
    # Otherwise process combo statistics and save #############################
        ...
        logger.debug(
            "Processing drug + cell combo statics dataframe ... "
