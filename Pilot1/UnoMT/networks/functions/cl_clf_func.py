...
def train_cl_clf(
    ...
    for batch_idx, (rnaseq, data_src, cl_site, cl_type, cl_category) in enumerate(
        data_loader
    ):
...
def valid_cl_clf(
    ...
    with torch.no_grad():
        for rnaseq, data_src, cl_site, cl_type, cl_category in data_loader:
            ...
            correct_category += (
                pred_category.eq(cl_category.view_as(pred_category)).sum().item()
            )
            correct_site += pred_site.eq(cl_site.view_as(pred_site)).sum().item()
            correct_type += pred_type.eq(cl_type.view_as(pred_type)).sum().item()
    ...
    print(
        ...
        "\n\t\tSite Accuracy: \t\t\t%5.2f%%; "
        "\n\t\tType Accuracy: \t\t\t%5.2f%%" % (category_acc, site_acc, type_acc)
    )
