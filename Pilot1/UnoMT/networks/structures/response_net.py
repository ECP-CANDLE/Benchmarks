...
class RespNet(nn.Module):
    def __init__(
        ...
        self.__resp_net.add_module(
            "dense_0", nn.Linear(gene_latent_dim + drug_latent_dim + 1, resp_layer_dim)
        )
        ...
        for i in range(1, resp_num_layers + 1):
            self.__resp_net.add_module(
                "dense_%d" % i, nn.Linear(resp_layer_dim, resp_layer_dim)
            )

            if resp_dropout > 0.0:
                self.__resp_net.add_module("dropout_%d" % i, nn.Dropout(resp_dropout))
    ...
    def forward(self, rnaseq, drug_feature, concentration):
        return self.__resp_net(
            torch.cat(
                (
                    self.__gene_encoder(rnaseq),
