...
class UnoMTModel(object):
    ...
    def build_data_loaders(self):
        ...
        self.drug_resp_val_loaders = [
            torch.utils.data.DataLoader(
                DrugRespDataset(
                    data_src=src, training=False, **(self.drug_resp_dataset_kwargs)
                ),
                batch_size=args.val_batch_size,
                **(self.dataloader_kwargs)
            )
            for src in args.val_sources
        ]

        self.cl_clf_trn_loader = torch.utils.data.DataLoader(
            CLClassDataset(training=True, **(self.cl_clf_dataset_kwargs)),
            batch_size=args.trn_batch_size,
            **(self.dataloader_kwargs)
        )

        self.cl_clf_val_loader = torch.utils.data.DataLoader(
            CLClassDataset(training=False, **(self.cl_clf_dataset_kwargs)),
            batch_size=args.val_batch_size,
            **(self.dataloader_kwargs)
        )

        self.drug_target_trn_loader = torch.utils.data.DataLoader(
            DrugTargetDataset(training=True, **(self.drug_target_dataset_kwargs)),
            batch_size=args.trn_batch_size,
            **(self.dataloader_kwargs)
        )

        self.drug_target_val_loader = torch.utils.data.DataLoader(
            DrugTargetDataset(training=False, **(self.drug_target_dataset_kwargs)),
            batch_size=args.val_batch_size,
            **(self.dataloader_kwargs)
        )

        self.drug_qed_trn_loader = torch.utils.data.DataLoader(
            DrugQEDDataset(training=True, **(self.drug_qed_dataset_kwargs)),
            batch_size=args.trn_batch_size,
            **(self.dataloader_kwargs)
        )

        self.drug_qed_val_loader = torch.utils.data.DataLoader(
            DrugQEDDataset(training=False, **(self.drug_qed_dataset_kwargs)),
            batch_size=args.val_batch_size,
            **(self.dataloader_kwargs)
        )
    ...
    def build_nn(self):
        ...
        self.resp_net = RespNet(
            ...
            resp_activation=args.resp_activation,
        ).to(device)
        ...
        self.drug_qed_net = RgsNet(
            ...
            activation=args.drug_qed_activation,
        ).to(device)
    ...
    def config_optimization(self):
        ...
        self.resp_lr_decay = LambdaLR(
            optimizer=self.resp_opt, lr_lambda=lambda e: args.lr_decay_factor**e
        )
        self.cl_clf_lr_decay = LambdaLR(
            optimizer=self.cl_clf_opt, lr_lambda=lambda e: args.lr_decay_factor**e
        )
        self.drug_target_lr_decay = LambdaLR(
            optimizer=self.drug_target_opt,
            lr_lambda=lambda e: args.lr_decay_factor**e,
        )
        self.drug_qed_lr_decay = LambdaLR(
            optimizer=self.drug_qed_opt, lr_lambda=lambda e: args.lr_decay_factor**e
        )

        self.resp_loss_func = F.l1_loss if args.resp_loss_func == "l1" else F.mse_loss
        self.drug_qed_loss_func = (
            F.l1_loss if args.drug_qed_loss_func == "l1" else F.mse_loss
        )
    ...
    def update_l2regularizer(self, reg):
        ...
        self.resp_opt = get_optimizer(
            opt_type=args.resp_opt,
            networks=self.resp_net,
            learning_rate=args.resp_lr,
            l2_regularization=reg,
        )

        self.cl_clf_opt = get_optimizer(
            opt_type=args.cl_clf_opt,
            networks=[self.category_clf_net, self.site_clf_net, self.type_clf_net],
            learning_rate=self.args.cl_clf_lr,
            l2_regularization=reg,
        )

        self.drug_target_opt = get_optimizer(
            opt_type=args.drug_target_opt,
            networks=self.drug_target_net,
            learning_rate=args.drug_target_lr,
            l2_regularization=reg,
        )

        self.drug_qed_opt = get_optimizer(
            opt_type=args.drug_qed_opt,
    ...
    def update_dropout(self, dropout_rate):
        ...
        self.resp_net = RespNet(
            ...
            resp_activation=self.args.resp_activation,
        ).to(self.device)
    ...
    def pre_train_config(self):
        ...
        print(
            "Data set: "
            + self.drug_resp_trn_loader.dataset.data_source
            + " Size: "
            + str(len(self.drug_resp_trn_loader.dataset))
        )
        print("Validation:")
    ...
    def train(self):
        ...
        for epoch in range(args.epochs):
            ...
            print(
                "Epoch Running Time: %.1f Seconds." % (time.time() - epoch_start_time)
            )
    ...
    def validation(self, epoch):
        ...
        cl_category_acc, cl_site_acc, cl_type_acc = valid_cl_clf(
            device=device,
            category_clf_net=self.category_clf_net,
            site_clf_net=self.site_clf_net,
            type_clf_net=self.type_clf_net,
        ...
        drug_target_acc = valid_drug_target(
            device=device,
            drug_target_net=self.drug_target_net,
        ...
        drug_qed_mse, drug_qed_mae, drug_qed_r2 = valid_drug_qed(
            device=device,
            drug_qed_net=self.drug_qed_net,
    ...
    def print_final_stats(self):
        ...
        print(
            "\t\tDrug Target Family \t Best Accuracy: %.3f%% (Epoch = %3d)"
            % (
        ...
        print(
            "\t\tDrug Weighted QED \t Best R2 Score: %+6.4f "
        ...
        # Print best predictions for drug classifiers and regressor
        print(
            "\t\tDrug Target Family \t Accuracy: %.3f%% "
            % (self.val_drug_target_acc[best_epoch])
        )

        print(
            "\t\tDrug Weighted QED \t R2 Score: %+6.4f "
            "(MSE = %8.6f, MAE = %6.6f)"
            % (
