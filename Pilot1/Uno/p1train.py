...
def get_parser(
    description="Run machine learning training algorithms implemented in scikit-learn",
):
    ...
    parser.add_argument("--cv", type=int, default=CV, help="cross validation folds")
    parser.add_argument(
        "--feature_subsample",
        type=int,
    ...
    parser.add_argument(
        "--balanced", action="store_true", help="balanced class weights"
    )
    parser.add_argument("--csv", action="store_true", help="comma separated file")
    parser.add_argument("--seed", type=int, default=SEED, help="specify random seed")
    return parser
...
def main():
    ...
    x, y, splits, features = split_data(
        df,
        ycol=args.ycol,
        ...
        groupcols=args.groupcols,
        ignore_categoricals=args.ignore_categoricals,
        verbose=True,
    )
    ...
    print(
        "Training the best model ({}={:.3g}) on the entire dataset...".format(
