...
LC2 = None
LC3 = None
LOCALLY_CONNECTED_LAYERS = [LC1, LC2, LC3]
...
def extension_from_parameters():
    ...
    if LOCALLY_CONNECTED_LAYERS:
        for i, n in enumerate(LOCALLY_CONNECTED_LAYERS):
            if n:
                ext += '.LC{}={}'.format(i+1, ','.join(map(str, n)))
        if POOL:
            ext += '.P={}'.format(POOL)
...
class RegressionDataGenerator(object):
    ...
    def flow(self, batch_size=32, val=False, reshape=False):
