...
def initialize_parameters(default_model="t29_default_model.txt"):
    t29_common = candle.Benchmark(
        file_path, default_model, "keras", prog="t29res.py", desc="resnet"
    )
...
def run(gParameters):
    ...
    checkpointer = ModelCheckpoint(
        filepath="t29res.autosave.model.h5",
        verbose=0,
    ...
    np.savetxt(
        "predict_train_classes.csv", predict_train_classes, delimiter=",", fmt="%d"
    )
    np.savetxt(
        "predict_test_classes.csv", predict_test_classes, delimiter=",", fmt="%d"
    )
