...
def load_data_pickle(path, gParameters):
...
def run(gParameters):
    ...
    X_train, Y_train, X_test, Y_test = load_data_pickle(file_train, gParameters)
...
# model.add(Activation('softmax'))
    ...
    model.save(path)
    path = '{}/{}.autosave.data.h5'.format(output_dir, model_name)
    pickle.dump( [X_train, Y_train, X_test, Y_test], open( path, "wb" ) )
    print(path)
    return history
